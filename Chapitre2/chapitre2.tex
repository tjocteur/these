...
\subparagraph{}Sur la \autoref{fig:predic_kay}-(a), nous représentons l'évolution obtenue de l'exposant de rugosité $\eta$ via un calcul de renormalisation dans une approximation à deux boucles \cite{le_doussal_two_loop_2002, wiese_longrange}. Celle-ci confirme bien l'attendu qualitatif d'une augmentation continue de cet exposant avec la portée $\alpha$, prenant place entre $\alpha = 4$ et $\alpha = 3$. Dans l'extension de la théorie CDP à LR-CDP, la correspondance entre les propriétés de rugosité et d'hyperuniformité se retrouve dans une nouvelle relation d'échelle entre les exposants $\eta$ et $\sigma$. En 2D, celle-ci prend la forme simple suivante \cite{wiese_longrange} :
...
\subparagraph{}La nécessité de s'approcher extrêmement près du point critique pour observer la criticalité de la limite thermodynamique pourrait aussi avoir un impact sur la détermination des exposants critiques\footnote{Dans \cite{barkema_universality_2003}, l'étude d'un modèle de particules similaire a montré que les effets de taille finie seuls pouvaient représenter une forte déviation à l'universalité attendue. Toutefois cette étude se base sur des tailles de système bien inférieures aux nôtres et nos résultats semblent indiquer que c'est la distance finie au point critique plutôt que la taille finie du système qui est responsable de la déviation dans notre cas.} présentée à la \autoref{sec:expcritjumps}. Cela pourrait expliquer pourquoi dans le cas identifié comme le plus défavorable $\alpha=4$, nous observons un écart à la limite de courte portée concernant les exposants $\beta$, $\gamma^\prime$ et $\delta$.
