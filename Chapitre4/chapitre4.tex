...
\noindent Dans l'espace réel continu, le calcul détaillé dans l'\annexeref{sec:TFinverse} donne la forme suivante pour l'interaction :
...
\begin{figure}[h]
	...
	\caption{(a) Courbes d'écoulement $\langle\dot{\gamma}\rangle = f(\delta\Sigma)$ pour les modèles $\alpha$-Picard $\alpha \in \{5, 4, 3, 2, 1.5, 1\}$ et pour le modèle Picard-CP. En encart, l'évolution associée des fluctuations du taux de cisaillement.
...
\noindent Nous cherchons donc d'abord à évaluer $K^\text{LR}_{u,v}$ pour un propagateur décroissant comme $1/r^\alpha$. Pour ce faire, de la même manière que pour la contrainte et l'activité, nous définissons un équivalent continu du propagateur discret selon :
...
\begin{equation}
    K^\text{LR}_{u,v} = \sum_{k,l = 1}^{L/a}G_{k,l}^\text{LR}(k,l)\frac{k^{2u}l^{2v}}{(2u)!(2v)!}
     \sim \left(\frac{L}{a}\right)^{2(1+u+v)-\alpha}\int \mathrm{d}\mathbf{r}~B_\alpha\frac{C_\alpha+\cos 4\theta}{r^\alpha}x^{2u}y^{2v},
\end{equation}
...
\begin{equation}
\begin{aligned}
    K^\text{LR}_{u,v} &\sim \left(\frac{L}{a}\right)^{2(1+u+v)-\alpha}\int_{-\pi}^\pi \mathrm{d}\theta~B_\alpha(C_\alpha+\cos 4\theta)\cos^2\theta\sin^2\theta \int_{a/L}^1 \mathrm{d}r ~ r^{1-\alpha+2(u+v)}\\
    &\sim\left(\frac{L}{a}\right)^{2(1+u+v)-\alpha}\frac{\pi}{8}B_\alpha(2C_\alpha-1)\int_{a/L}^1 \mathrm{d}r ~ r^{1-\alpha+2(u+v)}\\
...
\paragraph{Raisonnement plus simple dans l'espace réciproque}
...
\subsection[Influence de la portée sur les corrélations de contrainte dans l'état\\ stationnaire]{Influence de la portée sur les corrélations de contrainte dans l'état stationnaire}
...
\subparagraph{}Dans cette sous-sous-section, nous reprenons les résultats publiés dans \cite{jocteur_protocol_2025}. La publication complète est présentée dans l'\annexeref{sec:article2} mais ne sera pas complètement discutée ici afin de ne se concentrer que sur les informations essentielles pour notre propos.
