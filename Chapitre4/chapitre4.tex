...
\subsubsection{La nécessité d'un nouveau cadre à très longue portée}

\rem{Faire une partie pour dire : du coup pour alpha inf 4 on est pas très différent du depinning en principe dans la description continue, pourtant on a des résultats très fifférents. En fait il y a un autre cadre théroique que les eq de champ sur l'activité pour comprendre ces différences, c'est le cadre mu HL qu'on a déjà présenté dans les suspensions. Dans ce cas là ça donne ça. Donc ça permettrait bien d'expliquer la convexité, on retrouve bien le champ moyen à alpha 1. Donc on peut penser que c'est bien ça le bon mécanisme. Par contre Lin et Wyart ils ont remarqué que y'avait un bug, mais bon comme on a vu l'importance des modes 0 c'est pas si étonnant, on en parlera dans le chapitre suivant. }

\todo{Lier le bruit aux modes 0.}
...
\subparagraph{}Si ces comportements étaient déjà partiellement observés, cette étude permet de mettre en lumière ce point $\alpha \approx 3$ autour duquel le phénomène critique change drastiquement de paradigme par tous ses aspects. Dans la zone de démarcation, passé ce point, la transition semble totalement échapper au cadre de référence LR-CDP. Une toute nouvelle théorie semble alors nécessaire pour la description de la transition vers l'écoulement, différente de toutes les théories de champs connues.
